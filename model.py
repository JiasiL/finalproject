...
train_df = pd.read_csv('ACT4_competition_training_afterprocess.csv')
del train_df['MOLECULE']
train_y = np.array(train_df['Act'].tolist())
del train_df['Act']
train_x = train_df.as_matrix()
   
test_df = pd.read_csv('ACT4_competition_test_afterprocess.csv')
...
etr = ensemble.ExtraTreesRegressor(n_estimators=41, 
                                   bootstrap=False, 
                                   oob_score=False, 
                                   verbose=1,
                                   random_state=0,
